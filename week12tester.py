...
def test_partition_naive(size):
    ...
    [a, b, c] = week12AKA13.partition_naive(case, pivot)
...
def test_quicksort_naive(size):
    case = make_case(size)
    result = case[:]
    week12AKA13.quicksort_naive(result)
...
def test_quicksort_upgrade(size):
    case = make_case(size)
    result = case[:]
    week12AKA13.quicksort_upgrade(result)
    if result == sorted(case):
        return True
    else:
        raise AssertionError(
...
def test_partition_upgrade(size):
    ...
    [a, b, c] = week12AKA13.partition_upgrade(result, pivot, lo, hi)
    if (a + b + c) != (hi - lo):
        raise AssertionError(
